...
- [x] Re-warm + backtest — done 2026-07-16. **No wipe was needed:** the 7-day TTL had expired every row (last warm 07-08), so a plain warmer run refetched all 213 munis through the 6-cube v2 pipeline in ~4 min (206 scored, 7 without data; 400 rows all `score_version = 2`). Full backtest in **`docs/scoring-v2-backtest.md`** (v2 snapshot: `docs/snapshots/v2-scores-2026-07-16.json`). Headlines: distribution stable (mean 50.6→50.1, zero year drift across all 206 pairs); Msinga & Ratlou correctly unscored (unreliable figures); FH mean 38→50 (own-revenue fixes the rural penalty); Infra mean 80→73 (nobody maintains — not even CPT hits the 8% norm); Acc mean 62→52 (**118/196 munis waste ≥10% of opex**); eThekwini rises to #11 (best-maintaining metro), CPT edges it as top metro; losers are one profile — grant-funded builders who don't maintain (Jozini archetype). **Anchors confirmed, v2 is the live formula.**
- [x] prd.md rubric updated to v2 (2026-07-15; "provisional" hedge removed after the backtest, 2026-07-16)
- [ ] v2.1 calibration candidates (annual review, from the backtest): skip the UIFW blend when the audit sub-score is an earned 0 (statements outstanding shouldn't gain points from unaudited UIFW figures); verify `uifwexp` is per-year incurrence vs cumulative balance before hardening the 10% anchor
